angular
    ...
    .controller('ServiceOpenController', function ($rootScope, Restangular, $state, Upload, provider, providers, serviceTypes, regions, $filter, service, tags, ServiceService, leafletData, $window, service_languages, toasty, $scope, webClientUrl, confirmationLogs, DTOptionsBuilder, DTColumnDefBuilder, staticUrl) {
        ...
        vm.pullFromTransifex = function () {
            ServiceService
                ...
                .then(function (s) {
                    ...
                    toasty.success({title: 'Pulled from transifex!', msg: message, clickToClose: true, showClose: false, sound: false});
                })
                .catch(function (e) {
                    toasty.error({
            ...
            if (!vm.checkIfExists(newTag)) {
                Restangular
                    ...
                    .then(createdTag => {
                        vm
                            .tags
                            .push({'id': createdTag.id, 'name': createdTag.name});
                        let objectToPop = vm
                            .service
                            .tags
                            .filter(element => {
        ...
        vm.ckeditorOptions = () => {
            return {autoParagraph: false, allowedContent: true, height: 100};
        };
        vm.addContactInformation = () => {
            ...
            vm
                ...
                .push({'id': null, 'text': '', 'index': vm.service.contact_informations.length, 'type': ''})
        }
