    ...
    def get_same_coordinates_services(self, request, **kwargs):
        ...
        if requested_service_id:
            ...
            if requested_service.location:
                ...
                filtered_by_coordinates = filtered_by_region.annotate(distance=RawSQL('ST_Distance_Sphere(ST_GeomFromWKB(POINT%s, 4326),services_service.location)', (requested_service.location.coords,))).filter(distance__lt=max_distance_m).exclude(id=requested_service_id)                                                           
